function [hf_kspace] = grappa_main(slice_R,PE_R,ims,sg_kspace,acs,new_kspace,kernels,gui_on,par_on)
...
for klm=1:slice_R
    ...
    elseif(PE_R==4)
     ...
     elseif(PE_R==5)
        ...
        for outer = 1:PE_R-1
            for coil_selec = 1:size(conc_acs_fregion,3)
                if(outer==1)
                    selected_acs = conc_acs_fregion(row_start:row_end,mean(acq_cols(1:kernel_col))-2:mean(acq_cols_back(1:kernel_col))-2,coil_selec);
                elseif(outer==2)
                    selected_acs = conc_acs_fregion(row_start:row_end,mean(acq_cols(1:kernel_col))-1:mean(acq_cols_back(1:kernel_col))-1,coil_selec);
                elseif(outer==3)
                    selected_acs = conc_acs_fregion(row_start:row_end,mean(acq_cols(1:kernel_col))+0:mean(acq_cols_back(1:kernel_col))+0,coil_selec);
                elseif(outer==4)
                    selected_acs = conc_acs_fregion(row_start:row_end,mean(acq_cols(1:kernel_col))+1:mean(acq_cols_back(1:kernel_col))+1,coil_selec);
                end
                Mk(:,coil_selec,outer) = selected_acs(:);
    ...
    lambda = norm(MA'*MA,'fro')/sqrt(size(MA'*MA,1))*1e-2;
    %lambda = lambda*0;
    pre_cal_A = pinv(MA'*MA + lambda*eye(size(MA,2)));
    if(par_on==1)
...
for klm=1:slice_R
    ...
    if(par_on==1)
        for selected_cyc =1:ims
            tic
            disp(['Image at: ',num2str(selected_cyc)])
            ...
            elseif(PE_R==6)
                ...
                end_kspace = conv_op_ind_sg2_54(PE_R,ak_ind,mb_kspce,kernel_row,kernel_col,size(mb_kspce,1),size(mb_kspce,2),size(mb_kspce,3));
                mb_kspace_new1 = reshape(end_kspace,[size(mb_kspce,1) size(mb_kspce,2) size(mb_kspce,3)]);
                mb_kspace_new1 = circshift(mb_kspace_new1,[0 -2 0]);
                %% mid
                ak_ind = squeeze(data_ak_ind(klm,:,:,2));
                mb_kspce = squeeze(sg_kspace(:,:,:,klm,selected_cyc));
                end_kspace = conv_op_ind_sg2_54(PE_R,ak_ind,mb_kspce,kernel_row,kernel_col,size(mb_kspce,1),size(mb_kspce,2),size(mb_kspce,3));
                ...
                end_kspace = conv_op_ind_sg2_54(PE_R,ak_ind,mb_kspce,kernel_row,kernel_col,size(mb_kspce,1),size(mb_kspce,2),size(mb_kspce,3));
                mb_kspace_new3 = reshape(end_kspace,[size(mb_kspce,1) size(mb_kspce,2) size(mb_kspce,3)]);
                mb_kspace_new3 = circshift(mb_kspace_new3,[0 0 0]);
                %% right
                ak_ind = squeeze(data_ak_ind(klm,:,:,4));
                mb_kspce = squeeze(sg_kspace(:,:,:,klm,selected_cyc));
                end_kspace = conv_op_ind_sg2_54(PE_R,ak_ind,mb_kspce,kernel_row,kernel_col,size(mb_kspce,1),size(mb_kspce,2),size(mb_kspce,3));
                mb_kspace_new4 = reshape(end_kspace,[size(mb_kspce,1) size(mb_kspce,2) size(mb_kspce,3)]);
                mb_kspace_new4 = circshift(mb_kspace_new4,[0 1 0]);
                %% right
                ak_ind = squeeze(data_ak_ind(klm,:,:,5));
                mb_kspce = squeeze(sg_kspace(:,:,:,klm,selected_cyc));
                end_kspace = conv_op_ind_sg2_54(PE_R,ak_ind,mb_kspce,kernel_row,kernel_col,size(mb_kspce,1),size(mb_kspce,2),size(mb_kspce,3));
                mb_kspace_new5 = reshape(end_kspace,[size(mb_kspce,1) size(mb_kspce,2) size(mb_kspce,3)]);
                mb_kspace_new5 = circshift(mb_kspace_new5,[0 2 0]);
                
                
                mb_kspace_new = mb_kspace_new1+mb_kspace_new2+mb_kspace_new3+mb_kspace_new4+mb_kspace_new5;
            end
        ...
        for selected_cyc =1:ims
            ...
            elseif(PE_R==4)
             ...
             elseif(PE_R==5)
                ...
                mb_kspace_new = mb_kspace_new1+mb_kspace_new2+mb_kspace_new3+mb_kspace_new4;
            elseif(PE_R==6)
                %% left
                ak_ind = squeeze(data_ak_ind(klm,:,:,1));
                mb_kspce = squeeze(sg_kspace(:,:,:,klm,selected_cyc));
